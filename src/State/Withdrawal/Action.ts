...
export const withdrawalRequest = ({amount, jwt}) => async dispatch => {
    ...
    try {
        const response = await api.post(`/withdrawals/withdraw/${amount}`, null, {
...
export const addPaymentDetails = ({paymentDetails, jwt}) => async dispatch => {
    ...
    try {
        const response = await api.post(`/paymentDetails/add`, paymentDetails, {
            headers: { Authorization: `Bearer ${jwt}` }
        });
...
export const getPaymentDetails = ({ jwt}) => async dispatch => {
    ...
    try {
        const response = await api.get(`/paymentDetails/details`, {
            headers: { Authorization: `Bearer ${jwt}` }
        });
